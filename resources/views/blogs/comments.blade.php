@foreach ($data as $comment)
    <div class="single-comment">
        <div class="author">
            <img class="author-picture" src="{{ $comment->author->link_avatar }}" alt="{{ $comment->author->username }}">
        ...
        <div class="comment-detail">
            ...
            <form endpoint="{{ route('api.comment.store') }}" comment-parent="{{ $comment->id }}"
            ...
            <form method="post" action="{{ route('api.comment.update') }}" class="updateComment d-none">
                <input name="comment" value="{{ $comment->content }}" required />
                <div>
            ...
            <span class="color-time">
                {{ $comment->time_elapsed }} &nbsp;&nbsp;
            </span>

            <span class="color-time cursor-pointer totalReply {{ $comment->replies()->count() ? '' : 'd-none' }}"
                endpoint="{{ route('api.comment.reply', ['parentId' => $comment->id]) }}">
                {{ $comment->replies()->count() }} {{ __('message.reply') }}
            </span>
...
{{ $data->withQueryString()->links('layouts.pagination', ['useAjax' => true, 'blogId' => $blogId]) }}
