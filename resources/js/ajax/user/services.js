...
export default {
    ...
    handleCreateReplySuccess(endpoint, divElement) {
        $.ajax({
            url: endpoint,
            type: 'GET',
    ...
    handleDisplayTotalReply(totalReplyElement, incrementReply) {
        ...
        totalReplyElement.text(updateTotalReplyElement)
        totalReply ? totalReplyElement.removeClass('d-none') : totalReplyElement.addClass('d-none')
    },
    ...
    handleDecreaseComment(comment) {
        let totalComment = parseInt($('#totalComment').text());
        let newTotalCommentlement = $('<span>').attr('id', 'totalComment');
        let totalCommentUpdated = --totalComment;
        if (!comment.parent_id) {
            let commentElement = $('p[data-id="' + comment.id + '"]');
            let totalReplyElement = commentElement.siblings('span.totalReply');
            let totalReply = parseInt(totalReplyElement.text().match(/\d+/)[0]);
            totalCommentUpdated = totalComment - totalReply;
        }
        newTotalCommentlement.text(totalCommentUpdated);
        $('#totalComment').slideUp(function () {
            $(this).replaceWith(newTotalCommentlement);
            newTotalCommentlement.hide().slideDown();
        });
    ...
    createElement(comment, author, myComment = true) {
        let result =
            `<div class="author">
    ...
    handleLikeEvent(liked) {
        let totalLike = parseInt($('#totalLike').text());
        let newTotalLikeElement = $('<span>').attr('id', 'totalLike');
        liked ? newTotalLikeElement.text(totalLike - 1) : newTotalLikeElement.text(totalLike + 1);
